...
export async function checkAchievements(userId: string, gameStats: { 
  ...
  if (gameStats.completed && !userAchievements.some(ua => ua.achievement.name === ACHIEVEMENTS.FIRST_WIN.name)) {
    unlockedAchievements.push(await createAchievement(userId, ACHIEVEMENTS.FIRST_WIN))
  }

  // Verifica velocista
  if (gameStats.completed && gameStats.time < 30 && 
      !userAchievements.some(ua => ua.achievement.name === ACHIEVEMENTS.SPEED_DEMON.name)) {
  ...
  if (gamesPlayed >= 5 && !userAchievements.some(ua => ua.achievement.name === ACHIEVEMENTS.PERSISTENT.name)) {
  ...
  if (gameStats.completed && gameStats.mistakes === 0 && 
      !userAchievements.some(ua => ua.achievement.name === ACHIEVEMENTS.PERFECT_MEMORY.name)) {
...
async function createAchievement(userId: string, achievementData: any) {
